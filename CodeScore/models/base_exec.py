...
class CometModel(ptl.LightningModule, metaclass=abc.ABCMeta):
    ...
    def predict(
        ...
        passeds = torch.cat([pred.passeds for pred in predictions], dim=0).tolist()
        pass_at_1s = torch.cat([pred.pass_at_1s for pred in predictions], dim=0).tolist()
        ...
        if length_batching and gpus < 2:
            ...
            passeds = restore_list_order(passeds, sort_ids)
            pass_at_1s = restore_list_order(pass_at_1s, sort_ids)
            output = Prediction(scores=scores, passeds=passeds, pass_at_1s=pass_at_1s, system_score=sum(scores) / len(scores))
            if metadata:
            ...
            return Prediction(scores=scores, passeds=passeds, pass_at_1s=pass_at_1s, system_score=sum(scores) / len(scores))
